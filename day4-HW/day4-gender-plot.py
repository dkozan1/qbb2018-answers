...
#function to run through BOTH genders
def sex_abundance(gender):
	...
	compiled = {}
	#sex of interest, look for all the rows 
	#like what we put in our command line
	soi = df.loc[:, "sex"] == gender
	df = df.loc[soi,:]
	
	#each loop runs through for both genders
	for index, sample, sex, stage in df.itertuples():
		name = (stage)
		filename = os.path.join(sys.argv[3], sample, "t_data.ctab")
		ctab_df = pd.read_table(filename, index_col = "t_name")
		##transcriptID put into argument one, pulled out to make new datafram
